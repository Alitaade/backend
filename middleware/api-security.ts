...
export const corsMiddleware = (req: NextApiRequest, res: NextApiResponse, next: () => void) => {
  ...
  const isAllowedOrigin = allowedOrigins.includes(origin as string) || process.env.NODE_ENV === "development"
  ...
  res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization, X-API-Key, x-api-key, authorization")
