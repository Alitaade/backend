...
export const createVerificationToken = async (
  ...
  try {
    console.log(
      `Creating verification token for order ID: ${orderId}, order number: ${orderNumber}`
    );
...
export const verifyToken = async (
  ...
  try {
    console.log(
      `Verifying token for order number: ${orderNumber}, token: ${token}`
    );
    ...
    console.log(
      `Token usage count incremented. Current count: ${
        result.rows[0].usage_count + 1
      }`
    );
...
export const getTokenForOrder = async (
  orderNumber: string
): Promise<string | null> => {
  try {
...
export const getTokenUsageInfo = async (
  token: string
): Promise<{ usageCount: number; maxUses: number } | null> => {
  try {
    const result = await query(
