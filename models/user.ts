...
// Update the updateUser function to handle Google client ID when email is changed
  ...
  try {
    ...
    if (userData.email !== undefined) {
      ...
      if (currentUser.email !== userData.email && currentUser.google_id) {
        console.log(
          `Email changing from ${currentUser.email} to ${userData.email}, clearing Google ID`
        );
