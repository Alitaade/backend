...
export const verifyPaystackPayment = async (
  ...
  try {
    ...
    if (
      ...
      if (!orderId) {
        console.log(
          "No order ID in metadata, trying to extract from reference"
        );
        // Try to match ORDER-ORD-number pattern
        const orderIdMatch = (reference as string).match(/ORDER-(ORD-[^-]+)/);
      ...
      if (orderId) {
        ...
        console.log(`Looking for order with order_number: ${orderId}`);
        order = await getOrderByOrderNumber(orderId);

        // If exact match fails, try a partial match
        if (!order) {
          console.log(
            `Order not found with exact match, trying partial match for: ${orderId}`
          );
          // This assumes you have a function to find orders by partial order number
          const partialOrderNumber =
            orderId.split("-")[0] + "-" + orderId.split("-")[1];
          console.log(
            `Trying with partial order number: ${partialOrderNumber}`
          );
          order = await getOrderByOrderNumber(partialOrderNumber);
        }

        if (order) {
          console.log(
            `Found order with ID ${order.id} for order number ${orderId}`
          );
          ...
          // Update order status to completed instead of processing
          await updateOrderStatus(order.id, "completed");
          console.log(
            `Updated order status to 'completed' for order ${order.id}`
          );
          ...
          console.log(
            `Generated verification token: ${verificationToken.token} for order ${order.id}`
          );
        ...
        console.log(
          "Could not determine order ID from payment reference or metadata"
        );
      }
