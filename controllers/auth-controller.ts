...
export const googleAuth = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    if (!payload || !payload.email) {
      return res
        .status(400)
        .json({ error: "Invalid Google token or missing email" });
    }
    ...
    if (user) {
      ...
      if (user.email !== email) {
        console.log(
          `Google login: Email mismatch. User email: ${user.email}, Google email: ${email}`
        );
        ...
        if (existingUserWithEmail && existingUserWithEmail.id !== user.id) {
          ...
          return res.status(400).json({
            error:
              "Another account already exists with this email. Please log in with your updated email address.",
          });
        }
...
export const validateToken = async (
  ...
  try {
    ...
    if (bearer !== "Bearer" || !token) {
      ...
      return res
        .status(200)
        .json({ valid: false, error: "Invalid authorization format" });
    }
...
export const checkEmailAvailability = async (
  ...
  try {
    ...
    if (existingUser) {
      return res
        .status(400)
        .json({ available: false, error: "Email already in use" });
    }
